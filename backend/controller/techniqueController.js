...
const TechniqueController = {
    createTechnique: async (req, res) => {
        ...
        for (const translation of translations) {
            if (!translation.language || !translation.title || !translation.description) {
        ...
        try {
            const technique = await prisma.technique.create({
                data: {
                    image, // Опциональное поле для изображения
                    translations: {
            ...
            res.status(500).json({ error: error.message || 'Ошибка при создании техники' });
        }
    },
    ...
    updateTechnique: async (req, res) => {
        ...
        try {
            ...
            const technique = await prisma.technique.update({
                where: { id: Number(id) },
                data: {
                    image, // Обновление изображения (опционально)
                    translations: {
                        deleteMany: {}, // Удаляем старые переводы
                        create: translations.map(translation => ({
                            language: translation.language,
                            title: translation.title,
                            subtitle: translation.subtitle, // Обновление подзаголовка (опционально)
                            description: translation.description, // Обновление описания
