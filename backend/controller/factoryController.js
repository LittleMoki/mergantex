...
const FactoryController = {
    createFactory: async (req, res) => {
        ...
        if (!name || !Array.isArray(translations) || translations.length === 0) {
            return res
                .status(400)
    ...
    updateFactory: async (req, res) => {
        const { id } = req.params
        const { title, image, description, translations } = req.body
                ...
                data: {
                    ...
                    translations: {
                        ...
                        create: translations.map(translation => ({
                            language: translation.language,
                            title: translation.title,
                            name: translation.name, // Убедитесь, что name есть
                        })), // Создаём новые переводы
                    },
                },
            })
